...
function loadAllTiddlers() {
	return _(_.range(localStorage.length))
		.map((i) => {
...
PeerToPeerAdaptor.prototype.seedTiddler = function(tiddler) {
	let self = this;
	let buffer = new Buffer(JSON.stringify(tiddler));
...
PeerToPeerAdaptor.prototype.publishIndex = function() {
	...
	return Promise
		...
		.then(() => {
			let indexJsonBuf = new Buffer(JSON.stringify(self.index));
			self.indexTorrentClient = webTorrentClient();
		...
		.then((indexTorrent) => {
			...
			return dht.onAsync('ready')
				...
				.then((seq) => {
					...
					return dht.putAsync({
						k: publicKeyBuf,
						v: {indexInfoHash: new Buffer(indexTorrent.infoHash, 'hex')},
						seq: seq + 1,
						sign: function (buf) {
...
PeerToPeerAdaptor.prototype.getTiddlerInfo = function(tiddler) {
	var self = this;
	return self.filesystemAdaptor.getTiddlerInfo(tiddler);
};
...
PeerToPeerAdaptor.prototype.getSkinnyTiddlers = function(callback) {
	...
	return getIndexMetadata(dht)
		...
		.then(() => self.localStorageAdaptor.getSkinnyTiddlers())
		.asCallback(callback);
};
...
PeerToPeerAdaptor.prototype.loadTiddler = function(title,callback) {
	var self = this;
	self.filesystemAdaptor.loadTiddler(title, callback);
};
		...
		if(tiddlerInfoHash !== undefined) {
			...
			return self.tiddlersTorrentClient.removeAsync(tiddlerInfoHash)
				...
				.then(() => Promise.fromCallback(
					(callback) => self.filesystemAdaptor.deleteTiddler(title, callback, options)
				))
				.catch((e) => {
					console.log(e);
...
window.PeerToPeerAdaptor = PeerToPeerAdaptor;
